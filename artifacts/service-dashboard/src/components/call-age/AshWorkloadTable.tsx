...
export function AshWorkloadTable({
  rows,
  onAshClick,
}: {
  rows: CallAgeDashboard["topAshWorkload"];
  onAshClick?: (ash: string) => void;
}) {
  return (
    <Card className={ENTERPRISE_CARD}>
      <CardHeader className="pb-2">
        <CardTitle className="text-base">ASH Workload & Performance</CardTitle>
              ...
              {rows.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="h-24 text-center text-muted-foreground">
                    No ASH data for current filters
                  </TableCell>
                </TableRow>
              ) : (
                rows.map((row) => (
                  <TableRow
                    key={row.ash}
                    className={onAshClick ? "cursor-pointer hover:bg-muted/40" : undefined}
                    onClick={() => onAshClick?.(row.ash)}
                  >
                    <TableCell className="font-medium max-w-[160px] truncate" title={row.ash}>
                      {row.ash}
                    </TableCell>
